    ...
    describe(`s3-adapter ${o}`, () => {
        ...
        describe(`put ${o}`, () => {
            it(`put and get same value ${o}`, async () => {
                await adapter.put({ path: path.join(BUCKETS_NAMES.HKUBE, o, moment('2015-01-27').format(DateFormat), 'test1', 'test1.json'), data: { data: 'sss' } });
                await adapter.put({ path: path.join(BUCKETS_NAMES.HKUBE, o, moment('2015-01-26').format(DateFormat), 'test1', 'test1.json'), data: { data: 'sss' } });
                await adapter.put({ path: path.join(BUCKETS_NAMES.HKUBE, o, moment('2015-01-25').format(DateFormat), 'test1', 'test1.json'), data: { data: 'sss' } });
                await adapter.put({ path: path.join(BUCKETS_NAMES.HKUBE, o, moment('2015-01-24').format(DateFormat), 'test1', 'test1.json'), data: { data: 'sss' } });
                await adapter.put({ path: path.join(BUCKETS_NAMES.HKUBE, o, moment('2015-01-23').format(DateFormat), 'test1', 'test1.json'), data: { data: 'sss' } });
                await adapter.put({ path: path.join(BUCKETS_NAMES.HKUBE, o, moment('2015-01-22').format(DateFormat), 'test1', 'test1.json'), data: { data: 'sss' } });

                await adapter.put({ path: path.join(BUCKETS_NAMES.HKUBE, o, moment('2018-06-21').format(DateFormat), 'test1', 'test1.json'), data: { data: 'sss' } });
                await adapter.put({ path: path.join(BUCKETS_NAMES.HKUBE, o, moment('2018-06-22').format(DateFormat), 'test1', 'test1.json'), data: { data: 'sss' } });
                await adapter.put({ path: path.join(BUCKETS_NAMES.HKUBE, o, moment('2018-06-23').format(DateFormat), 'test1', 'test1.json'), data: { data: 'sss' } });
                await adapter.put({ path: path.join(BUCKETS_NAMES.HKUBE, o, moment('2018-06-24').format(DateFormat), 'test1', 'test1.json'), data: { data: 'sss' } });
                await adapter.put({ path: path.join(BUCKETS_NAMES.HKUBE, o, moment('2018-06-25').format(DateFormat), 'test1', 'test1.json'), data: { data: 'sss' } });

                const link = await adapter.put({ path: path.join(BUCKETS_NAMES.HKUBE, o, moment().format(DateFormat), Date.now().toString(), 'task-1'), data: 'test' });
                const res = await adapter.get(link);
                expect(res).to.equal('test');
            });
            it(`put result ${o}`, async () => {
            ...
            it(`get all tasks of specific jobid ${o}`, async () => {
            ...
            it(`get more than 3000 items ${o}`, async () => {
                const promiseArray = [];
            ...
            it(`delete more than 3000 items ${o}`, async () => {
                {
                    const promiseArray = [];
                    for (let i = 0; i < 3500; i += 1) {
            ...
            it(`delete by date more than 3000 items ${o}`, async () => {
                const promiseArray = [];
                for (let i = 0; i < 3500; i += 1) {
            ...
            it(`list objects without prefix ${o}`, async () => {
            ...
            it(`list objects with delimiter ${o}`, async () => {
                const jobId = Date.now().toString();
