...
class S3Adapter {
    ...
    async list(options) {
        const parsedPath = this._parsePath(options.path);
        const result = await S3Client.listObjects({ Bucket: parsedPath.Bucket, Prefix: parsedPath.Key });
        return result.map(x => ({ path: path.join(parsedPath.Bucket, x.Key) }));
    }

    async listWithStats(options) {
        const parsedPath = this._parsePath(options.path);
        const result = await S3Client.listObjectsWithStats({ Bucket: parsedPath.Bucket, Prefix: parsedPath.Key });
    ...
    async listPrefixes(options) {
        const parsedPath = this._parsePath(options.path);
        const res = await S3Client.listByDelimiter({ Bucket: parsedPath.Bucket, Delimiter: '/' });
    ...
    async getStream(options) {
        const parsedPath = this._parsePath(options.path);
        return S3Client.getStream(parsedPath);
    }
