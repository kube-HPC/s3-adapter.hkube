...
class S3Adapter {
    ...
    /**
     * This method get an array of buffers and upload them in a single request/
     * For now it just concat the buffers into memory, but in the future we want
     * to upload each part without combine them into memory.
     */
    async append(option) {
        const options = { ...this._parsePath(option.path), Body: option.data };
        const params = validator.validateAndReplacePutParams(options);
        ...
        const result = await this._client.upload({ ...params, Body }).promise();
        if (this._bucketName) {
            return { path: result.Key };
        }
        return { path: path.join(result.Bucket, result.Key) };
