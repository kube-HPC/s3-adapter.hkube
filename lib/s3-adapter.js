...
class S3Adapter {
    ...
    async init(connection, log) {
        if (!this._wasInit) {
            ...
            this._wasInit = true;
            this._log = log;
            S3Client.createBucket({ Bucket: 'hkube' });
            S3Client.createBucket({ Bucket: 'hkube-results' });
        }
    }

    async putResults(options) {
        return this._put({ Bucket: 'hkube-results', Key: `${moment().format(DATE_FORMAT)}/${options.jobId}/result.json`, Body: options.data });
    }

    async put(options) {
        return this._put({ Bucket: 'hkube', Key: `${moment().format(DATE_FORMAT)}/${options.jobId}/${options.taskId}`, Body: options.data });
    }
