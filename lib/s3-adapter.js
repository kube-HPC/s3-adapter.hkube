...
class S3Adapter {
    ...
    async put(options) {
        ...
        const end = Date.now();
        if (this._log)
            this._log.debug(`Execution of put takes ${end - start}ms`, { component });
    ...
    async get(options) {
        ...
        if (this._log)
            this._log.debug(`Execution of get takes ${end - start}ms`, { component });
    ...
    async jobPath(options) {
        ...
        const end = Date.now();
        if (this._log)
            this._log.debug(`Execution of createBucket takes ${end - start}ms`, { component });
