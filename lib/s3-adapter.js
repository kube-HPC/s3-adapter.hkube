...
class S3Adapter {
    ...
    async deleteByDate(options) {
        ...
        const objectsToDelete = await S3Client.listObjects({ Bucket: BUCKETS_NAMES.HKUBE, Prefix: dateToDelete });
        objectsToDelete.push({ Key: dateToDelete });
        return S3Client.deleteObjects({ Bucket: BUCKETS_NAMES.HKUBE, Delete: { Objects: objectsToDelete } });
    }

    async deleteResultsByDate(options) {
        const dateToDelete = moment(options.Date).format(DATE_FORMAT);
        const objectsToDelete = await S3Client.listObjects({ Bucket: BUCKETS_NAMES.HKUBE_RESULTS, Prefix: dateToDelete });
        objectsToDelete.push({ Key: dateToDelete });
        return S3Client.deleteObjects({ Bucket: BUCKETS_NAMES.HKUBE_RESULTS, Delete: { Objects: objectsToDelete } });
    }
